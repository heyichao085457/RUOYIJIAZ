<template>
  <div class="container">
    <div class="line"></div>
    <div class="sellect">
      <el-form ref="form" :model="formDatade" label-width="55px">
        <div>
          <el-row class="title" type="flex" align="middle">
            ...
            <el-col :span="4" :style="{ marginLeft: '15px' }">
              <el-input
                ...
                v-model.trim="formDatade.dkOrTb"
                clearable
    ...
    <div class="table-container">
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <el-tooltip
              :content="scope.row.qbtbHchpdlx"
              placement="top"
              effect="dark"
            >
              <div class="address-cell">
                {{ scope.row.qbtbHchpdlx }}
              </div>
            </el-tooltip>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    saveFormData() {
      // 创建formDatahut的副本
      let dataToStore = { ...this.formDatade };
      ...
      localStorage.setItem("formDatade", JSON.stringify(dataToStore));
    },
    loadFormData() {
      const savedData = localStorage.getItem("formDatade");
      if (savedData) {
        this.formDatade = JSON.parse(savedData);
      }
    },
    // saveFormData() {
    //   localStorage.setItem("formDatade", JSON.stringify(this.formDatade));
    // },
    // loadFormData() {
    //   const savedData = localStorage.getItem("formDatade");
    //   if (savedData) {
    ...
    beforeRouteEnter(to, from, next) {
      next((vm) => {
        if (to.params.savedData) {
          vm.formDatade = to.params.savedData;
        }
      });
    },
    handleDateChange(dates) {
      if (Array.isArray(dates) && dates.length > 0) {
        ...
        this.formDatade.startDate = formattedDates[0];
        this.formDatade.endDate = formattedDates[1];
      } else {
        this.formDatade.startDate = "";
        this.formDatade.endDate = "";
      }
    },
    emitGetData(data) {
      // console.log(data);
      this.formDatade.sfwttb = data.sfwttb;
      this.formDatade.hcjd = data.hcjd;
      this.formDatade.pdlx = data.tblx;
      this.formDatade.shengjidxjg = data.checkMethod;
      this.formDatade.shijidxjg = data.illegalityType;
      this.formDatade.qbtbHchpdlx = data.hchlxx;
      this.formDatade.qbtbSfnrwfjs = data.deductionType;
      this.formDatade.qbtbTbly = data.tbly;
      this.formDatade.tbfl = data.tbfl;
      this.formDatade.qbtbCancelStatus = data.qbtbCancelstatus;

      this.formDatade.hcjd = data.hcjd;
      this.formDatade.xmlx = data.xmlx;
      this.formDatade.zglsqk = data.zglsqk;
      this.formDatade.qtjtqx = data.qtjtqx;
      this.formDatade.hfjtqx = data.hfjtqx;
      this.formDatade.wfsjyt = data.sjyt;

      const updateFormData = (field, value, condition) => {
        switch (condition) {
          case "3":
            this.formDatade[field] = value;
            break;
          case "1":
            this.formDatade[`${field}Max`] = value;
            break;
          case "2":
            this.formDatade[`${field}Min`] = value;
            break;
          case "4":
            this.formDatade[`dydy${field}`] = value;
            break;
          case "5":
            this.formDatade[`xydy${field}`] = value;
            break;
          case "6":
            const [min, max] = value.split("~");
            this.formDatade[`${field}Max`] = max;
            this.formDatade[`${field}Min`] = min;
            break;
        }
      };
      ...
      if (data.freck.condition === "4") {
        this.formDatade.dydygdmj = data.freck.value;
      } else if (data.freck.condition === "5") {
        this.formDatade.xydygdmj = data.freck.value;
      } else {
        updateFormData("gdmj", data.freck.value, data.freck.condition);
      }

      if (data.Cufreck.condition === "4") {
        this.formDatade.dydynydmj = data.Cufreck.value;
      } else if (data.Cufreck.condition === "5") {
        this.formDatade.xydynydmj = data.Cufreck.value;
      } else {
    ...
    qualityinspection() {
      ...
      formData.append("file", this.selectedFile);
      this.loadingxlsx = true;
      importWyhc2022(formData).then((res) => {
        if (res.code === 200) {
          this.loadingxlsx = false;
          this.checkListt();
          this.$message.success("上传成功");
    ...
    listfileList(params) {
      fileListlist2022(params).then((res) => {
    ...
    terDialog() {
      if (this.namerow == "") {
        ...
        this.formDatade.remarks = this.namerow;
        this.formDatade.size = this.total;
        let data = { ...this.formDatade };

        addJsc2022(data).then((res) => {
          if (res.code === 200) {
            this.checkListt();
            this.$message({
              message: "添加成功",
    ...
    handleEditClick(value) {
      ...
      // console.log(value);
      this.$router.push({
        path: "/2022xiang/2022FusionLedgerxiang",
        query: {
          // id: value.id,
    ...
    handleCurrentChange(value) {
      this.formDatade.pageNum = value;
      this.findAll();
    },
    handleSizeli(value) {
      this.formDatade.pageSize = value;
      this.findAll();
    },
    ...
    handleChangeXzq(value) {
      this.formDatade.city = "";
      this.formDatade.county = "";
      if (value.length) {
        const textArr = [];
        ...
        this.formDatade.city = arr.join(",");
        this.formDatade.county = county.join(",");
      }
    },
    //数组转对象
    arrayToObject(arr) {
    ...
    findAll(isSearch) {
      ...
      // }
      if (this.formDatade.snapdate) {
        this.formDatade.snapdate = this.formatDate(this.formDatade.snapdate);
      }

      const { selectXZQ, valuetime, ...params } = this.formDatade;
    ...
    clear() {
      ...
      this.formDatade.year = ye;
      this.month1 = [];
      this.formDatade.selectXZQ = [];
      this.findAll();
    },
    async checkListt() {
      try {
        let ok = await computeList2022();
