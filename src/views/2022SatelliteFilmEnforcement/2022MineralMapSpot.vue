<template>
  <div class="container">
    <div class="line"></div>
    <div class="sellect">
      <el-form ref="form" :model="formDatahyu" label-width="55px">
        <div>
          <el-row class="title" type="flex" align="middle">
            ...
            <el-col :span="4" :style="{ marginLeft: '15px' }">
              <el-input
                ...
                v-model.trim="formDatahyu.dkOrTb"
                clearable
...
export default {
  ...
  methods: {
    ...
    saveFormData() {
      localStorage.setItem("formDatahyu", JSON.stringify(this.formDatahyu));
    },
    loadFormData() {
      const savedData = localStorage.getItem("formDatahyu");
      if (savedData) {
        this.formDatahyu = JSON.parse(savedData);
      }
    },
    ...
    beforeRouteEnter(to, from, next) {
      next((vm) => {
        if (to.params.savedData) {
          vm.formDatahyu = to.params.savedData;
        }
      });
    },
    handleDateChange(dates) {
      if (Array.isArray(dates) && dates.length > 0) {
        ...
        this.formDatahyu.startDate = formattedDates[0];
        this.formDatahyu.endDate = formattedDates[1];
      } else {
        this.formDatahyu.startDate = "";
        this.formDatahyu.endDate = "";
      }
    },
    emitGetData(data) {
      // console.log(data);
      this.formDatahyu.sfwttb = data.sfwttb;
      this.formDatahyu.hcjd = data.hcjd;
      this.formDatahyu.pdlx = data.tblx;
      this.formDatahyu.shengjidxjg = data.checkMethod;
      this.formDatahyu.shijidxjg = data.illegalityType;
      this.formDatahyu.qbtbHchpdlx = data.hchlxx;
      this.formDatahyu.qbtbSfnrwfjs = data.deductionType;
      this.formDatahyu.qbtbTbly = data.tbly;
      this.formDatahyu.tbfl = data.tbfl;
      this.formDatahyu.qbtbCancelStatus = data.qbtbCancelstatus;

      this.formDatahyu.hcjd = data.hcjd;
      this.formDatahyu.xmlx = data.xmlx;
      this.formDatahyu.zglsqk = data.zglsqk;
      this.formDatahyu.qtjtqx = data.qtjtqx;
      this.formDatahyu.hfjtqx = data.hfjtqx;
      this.formDatahyu.wfsjyt = data.sjyt;

      const updateFormData = (field, value, condition) => {
        switch (condition) {
          case "3":
            this.formDatahyu[field] = value;
            break;
          case "1":
            this.formDatahyu[`${field}Max`] = value;
            break;
          case "2":
            this.formDatahyu[`${field}Min`] = value;
            break;
          case "4":
            this.formDatahyu[`dydy${field}`] = value;
            break;
          case "5":
            this.formDatahyu[`xydy${field}`] = value;
            break;
          case "6":
            const [min, max] = value.split("~");
            this.formDatahyu[`${field}Max`] = max;
            this.formDatahyu[`${field}Min`] = min;
            break;
        }
      };
      ...
      if (data.freck.condition === "4") {
        this.formDatahyu.dydygdmj = data.freck.value;
      } else if (data.freck.condition === "5") {
        this.formDatahyu.xydygdmj = data.freck.value;
      } else {
        updateFormData("gdmj", data.freck.value, data.freck.condition);
      }

      if (data.Cufreck.condition === "4") {
        this.formDatahyu.dydynydmj = data.Cufreck.value;
      } else if (data.Cufreck.condition === "5") {
        this.formDatahyu.xydynydmj = data.Cufreck.value;
      } else {
    ...
    terDialog() {
      if (this.namerow == "") {
        ...
        this.formDatahyu.remarks = this.namerow;
        let data = { ...this.formDatahyu };
    ...
    exportData() {
      ...
      special2022kctb(this.formDatahyu)
        .then((res) => {
          if (res.code === 200) {
    ...
    handleCurrentChange(value) {
      this.formDatahyu.pageNum = value;
      this.findAll();
    },
    handleSizeli(value) {
      this.formDatahyu.pageSize = value;
      this.findAll();
    },
    ...
    handleChangeXzq(value) {
      this.formDatahyu.city = "";
      this.formDatahyu.county = "";
      if (value.length) {
        const textArr = [];
        ...
        this.formDatahyu.city = arr.join(",");
        this.formDatahyu.county = county.join(",");
      }
    },
    //数组转对象
    arrayToObject(arr) {
    ...
    findAll(isSearch) {
      ...
      // }
      if (this.formDatahyu.snapdate) {
        this.formDatahyu.snapdate = this.formatDate(this.formDatahyu.snapdate);
      }

      const { selectXZQ, valuetime, ...params } = this.formDatahyu;
      kctb2022(params)
        .then((res) => {
          if (res.code == 200) {
            load.close();
