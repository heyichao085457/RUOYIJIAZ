...
export default {
  ...
  methods: {
    ...
    saveFormData() {
      // 创建formDatahut的副本
      let dataToStore = { ...this.formData1 };
      ...
      localStorage.setItem("formData1", JSON.stringify(dataToStore));
    },
    loadFormData() {
      const savedData = localStorage.getItem("formData1");
      if (savedData) {
        this.formData1 = JSON.parse(savedData);
      }
    },
    // saveFormData() {
    ...
    beforeRouteEnter(to, from, next) {
      next((vm) => {
        if (to.params.savedData) {
          vm.formData1 = to.params.savedData;
        }
      });
    },
    ...
    emitGetData(data) {
      // console.log(data);
      this.formData1.tblx = data.tblx;
      this.formData1.tbhcjd = data.tbhcjd;
      this.formData1.hczt = data.hczt;
      this.formData1.jzzt = data.jzzt;
      this.formData1.hchlx = data.hchlxx;
      this.formData1.wfzl = data.wfzl;
      this.formData1.zzyj = data.zzyj;
      this.formData1.wyhcCancelStatus = data.wyhcCancelStatus;
      this.formData1.wyhcSfnrwfjs = data.wyhcSfnrwfjs;
    ...
    terDialog() {
      this.formData1.remarks = this.namerow;
    ...
    exportData() {
      ...
      exportBylandCodeList2022(params).then((ok) => {
        if (ok.code == 200) {
    ...
    handleEditClick(value) {
      this.$router.push({
        path: "/2022xiang/Provincialverificationin2022xiang",
        query: {
          // id: value.id,
    ...
    findAll(isSearch) {
      ...
      // }
      if (this.formData1.snapdate) {
        this.formData1.snapdate = this.formatDate(this.formData1.snapdate);
      }

      const { selectXZQ, valuetime, ...params } = this.formData1;
      listWyhc2022(params)
        .then((res) => {
