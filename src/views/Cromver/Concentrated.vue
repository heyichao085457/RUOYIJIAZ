<template>
  <div class="container">
    <div class="line"></div>
    <div class="sellect">
      <el-form ref="form" :model="formData9" label-width="55px">
        <div>
          <el-row class="title" type="flex" align="middle">
            ...
            <el-col :span="4" :style="{ marginLeft: '15px' }">
              <el-input
                ...
                v-model.trim="formData9.freckleCode"
                clearable
    ...
    <div class="table-container">
      <el-table 
        ...
        height="600"
        :data="tableData"
        @selection-change="deletevaluepo"
      >
        ...
        >
          <template slot-scope="scope">
            <el-tooltip :content="scope.row.wtlx" placement="top" effect="dark">
              <div class="address-cell">
                {{ scope.row.wtlx }}
              </div>
            </el-tooltip>
      ...
      <div style="width: 50%; height: 30px">
        <el-row class="title" type="flex" align="middle">
          <div :style="{ marginLeft: '10px' }">
            <el-button size="mini" type="primary" @click="deletek"
              >删除</el-button
            >
          </div>
          <div
            :style="{ marginLeft: '10px' }"
            v-if="permissions['集中举证批量销号'] === 0"
          >
            <el-button size="mini" type="primary" @click="importData"
              >批量销号</el-button
            >
          </div>
          <div
            :style="{ marginLeft: '10px' }"
            v-if="permissions['集中举证模板'] === 0"
          >
            <el-button size="mini" type="primary" @click="beforeUploadjzjz"
              >集中举证模板</el-button
            >
          </div>
      ...
      <div class="dialoglbodylo">
        ...
        >
          <div>
            ...
            <el-select v-model="hcrwmc" placeholder="请选择核查任务名称">
              <el-option
          ...
          <!-- <el-button type="primary" @click="beforeUpload">数据质检</el-button> -->
          <el-button
            type="primary"
            style="margin-top: 15px"
            @click="qualityinspection"
            >上传</el-button
          >
    ...
    <div class="pole">
      ...
      <script>
import screen from "../Enforcementxiang/Concentscreen.vue";
...
export default {
  ...
  methods: {
    ...
    deletek() {
      let ids = this.deleteIdop.slice();
      jzjzdelete(ids).then((ok) => {
        if (ok.code == 200) {
          this.$message({
            message: "删除成功",
            type: "success",
    ...
    tdwpWjjListt() {
      hcrwlist2024().then((ok) => {
        ...
        ok.data.forEach((item) => {
          this.optionsstyle.push({ value: item.hcrwmc, label: item.hcrwmc });
        });
      });
    },
    saveFormData() {
      // 创建formDatahut的副本
      let dataToStore = { ...this.formData9 };
      ...
      localStorage.setItem("formData9", JSON.stringify(dataToStore));
    },
    loadFormData() {
      const savedData = localStorage.getItem("formData9");
      if (savedData) {
        this.formData9 = JSON.parse(savedData);
      }
    },
    // saveFormData() {
    //   localStorage.setItem("formData9", JSON.stringify(this.formData9));
    // },
    // loadFormData() {
    //   const savedData = localStorage.getItem("formData9");
    //   if (savedData) {
    ...
    beforeRouteEnter(to, from, next) {
      next((vm) => {
        if (to.params.savedData) {
          vm.formData9 = to.params.savedData;
        }
      });
    },
    handleDateChange(dates) {
      if (Array.isArray(dates) && dates.length > 0) {
        ...
        this.formData9.startDate = formattedDates[0];
        this.formData9.endDate = formattedDates[1];
      } else {
        this.formData9.startDate = "";
        this.formData9.endDate = "";
      }
    },
    emitGetData(data) {
      // console.log(data);
      this.formData9.jzjzPdlx = data.jzjzPdlx;
      this.formData9.wtlx = data.wtlx;
      this.formData9.jzjzHchlx = data.jzjzHchlx;
      this.formData9.cancelStatus = data.cancelStatus;
      this.formData9.jzjzSfnrwfjs = data.jzjzSfnrwfjs;
      this.formData9.jzjzZzyj = data.jzjzZzyj;
      this.formData9.hcrwName = data.hcrwmc;
      this.formData9.sfxh = data.SFXH;
      this.formData9.jzjd = data.jzjd;
      this.findAll(true);
    },
    snapdatea() {
    ...
    beforeUploadjzjz() {
      window.open("http://124.114.203.222:8084/downloads/销号模板.xlsx");
    },
    qualityinspection() {
      if (this.hcrwmc == "" && !this.selectedFile) {
        this.$message({
          message: "请选择核查任务名称并选择一个文件",
          type: "warning",
        });
        // if (!this.selectedFile) {
        //   this.$message.error("请先选择一个文件");
        ...
        let formData = new FormData();
        formData.append("file", this.selectedFile);
        formData.append("hcrwmc", this.hcrwmc);
        // console.log(this.selectedFile);
        // let params = {
        //   file: this.selectedFile,
        //   hcrwmc: this.valuestyle,
        // };
        this.loadingxlsx = true;
        batchAddXhtb(formData).then((res) => {
          if (res.code === 200) {
            this.loadingxlsx = false;
            this.$message.success("上传成功");
            this.fileList = [];
    ...
    terDialog() {
      this.formData9.remarks = this.namerow;
      let data = { ...this.formData9 };
    ...
    exportData() {
      ...
      let exportRhtz2024 = {
        city: this.formData9.city,
        county: this.formData9.county,
        ssqx: this.formData9.ssqx,
        cancelStatus: this.formData9.cancelStatus,
        checkDetermineResult: this.formData9.checkDetermineResult,
        startDate: this.formData9.startDate,
        endDate: this.formData9.endDate,
        checkJobName: this.formData9.checkJobName,
        dkOrTb: this.formData9.dkOrTb,
        snapdate: this.formData9.snapdate,
        checkMethod: this.formData9.checkMethod,
        sfwttb: this.formData9.sfwttb,
        pdlx: this.formData9.pdlx,
    ...
    handleEditClick(value) {
      ...
      //   duration: 0,
      // });
      let idrower = value.id;
      // console.log(idrower);
      this.$router.push({
        path: "/Enforcementxiang/Concentratedxiang",
        query: {
          id: value.id,
    ...
    handleChangeXzq(value) {
      ...
      if (value.length) {
        ...
        this.formData9.city = arr.join(",");
        this.formData9.county = county.join(",");
      }
    },
    //数组转对象
    arrayToObject(arr) {
    ...
    findAll(isSearch) {
      ...
      // }
      if (this.formData9.snapdate) {
        this.formData9.snapdate = this.formatDate(this.formData9.snapdate);
      }

      const { selectXZQ, valuetime, ...params } = this.formData9;
