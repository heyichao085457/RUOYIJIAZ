<template>
  <div class="container">
    <div class="line"></div>
    <div class="sellect">
      <el-form ref="form" :model="formDataLIstr" label-width="55px">
        <div>
          <el-row class="title" type="flex" align="middle">
            <el-col :span="4">
              <el-form-item label="行政区">
                <el-cascader
                  ref="cascader"
                  clearable
                  v-model="formDataLIstr.selectXZQ"
                  :options="xzq"
                  :style="{ width: '100%' }"
                  :props="{
                    value: 'value',
            ...
            <el-col :span="5">
              <el-date-picker
                v-model="formDataLIstr.valuetime"
                type="daterange"
                range-separator="至"
            ...
            <!-- <el-col :span="3" :style="{ marginLeft: '10px' }">
              <el-date-picker
                v-model="formDataLIstr.snapdate"
                type="date"
            ...
            <el-col :span="4" :style="{ marginLeft: '15px' }">
              <el-input
                ...
                v-model.trim="formDataLIstr.dkOrTb"
                clearable
    ...
    <div class="flexlist">
      <div style="width: 50%; height: 30px">
        <el-row class="title" type="flex" align="middle">
          ...
          <!-- <div :style="{ marginLeft: '10px' }">
            <el-button type="primary" size="mini" @click="exportBylandCod"
              >外业成果清单</el-button
            >
          </div> -->
          <div :style="{ marginLeft: '10px' }">
            <el-dropdown @command="dropdownItemClick">
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item
                  ...
                  v-if="permissions['外业成果清单'] === 0"
                  >成果清单列表</el-dropdown-item
                >
                <el-dropdown-item
      ...
      <div style="width: 50%">
        <el-pagination
          ...
          :page-sizes="[20, 1000, 2000, 3000]"
          :page-size="formDataLIstr.pageSize"
      ...
      >
        ...
        <div id="Dataer">
          <el-table
            ...
            style="width: 100%"
            height="500px"
            v-loading="loadingChanpoi"
            @selection-change="handleSelece2"
          >
            <el-table-column type="selection" width="55"> </el-table-column>
            <el-table-column type="index" label="序号" width="80">
            </el-table-column>
          ...
          <div>
            <div>
              <el-button type="primary" @click="shanliop2">删除</el-button>
            </div>
            <!-- <div class="block" style="width: 50%">
              <el-pagination
                @size-change="handleSizeChange"
                @current-change="handleCurrentChangerow1"
                :current-page="currentPagerow4"
                :page-sizes="[20, 50, 100]"
                :page-size="pageSizerow"
                layout="total, sizes, prev, pager, next, jumper"
                :total="allDatarow.length"
              >
              </el-pagination>
            </div> -->
          </div>
    ...
    <script>
import screen from "../Enforcementxiang/Fieldscreen.vue";
...
export default {
  ...
  methods: {
    handleSizeli(value) {
      ...
      this.formDataLIstr.pageSize = value;
      this.findAll();
    },
    handleSelece(val) {
      // console.log(val, "val");
    ...
    saveFormData() {
      // 创建formDatahut的副本
      let dataToStore = { ...this.formDataLIstr };
      ...
      localStorage.setItem("formDataLIstr", JSON.stringify(dataToStore));
    },
    loadFormData() {
      const savedData = localStorage.getItem("formDataLIstr");
      if (savedData) {
        this.formDataLIstr = JSON.parse(savedData);
      }
    },
    // saveFormData() {
    //   localStorage.setItem("formDataLIstr", JSON.stringify(this.formDataLIstr));
    // },
    // loadFormData() {
    //   const savedData = localStorage.getItem("formDataLIstr");
    //   if (savedData) {
    ...
    beforeRouteEnter(to, from, next) {
      next((vm) => {
        if (to.params.savedData) {
          vm.formDataLIstr = to.params.savedData;
        }
      });
    },
    handleDateChange(dates) {
      if (Array.isArray(dates) && dates.length > 0) {
        ...
        this.formDataLIstr.startDate = formattedDates[0];
        this.formDataLIstr.endDate = formattedDates[1];
      } else {
        this.formDataLIstr.startDate = "";
        this.formDataLIstr.endDate = "";
      }
    },
    emitGetData(data) {
      // console.log(data);
      this.formDataLIstr.hcpcjsj = data.hcpcjsj;
      this.formDataLIstr.tblx = data.tblx;
      this.formDataLIstr.tbhcjd = data.tbhcjd;
      this.formDataLIstr.hczt = data.hczt;
      this.formDataLIstr.jzzt = data.jzzt;
      this.formDataLIstr.hchlx = data.hchlxx;
      this.formDataLIstr.wfzl = data.wfzl;
      this.formDataLIstr.zzyj = data.zzyj;
      this.formDataLIstr.wyhcCancelStatus = data.wyhcCancelStatus;
      this.formDataLIstr.wyhcSfnrwfjs = data.wyhcSfnrwfjs;
    ...
    // },
    beforeUploadjzjz() {
      window.open("http://124.114.203.222:8084/downloads/外业核查模板.xlsx");
    },
    qualityinspection() {
      if (!this.selectedFile) {
        this.$message.error("请先选择一个文件");
    ...
    terDialog() {
      this.formDataLIstr.remarks = this.namerow;
      let data = { ...this.formDataLIstr };
    ...
    exportData() {
      this.loadingChan = true;
      // let landCodes = this.selectedRows.map((row) => row.landCode).join(",");
      // let freckleCodes = this.selectedRows;
      ...
      // let params = {
      //   landCode: landCodes,
      //   freckleCode: freckleCodes,
      //   ids: ids,
      // };
      exportBylandCode(this.formDataLIstr).then((ok) => {
        if (ok.code == 200) {
          this.loadingChan = false;
          this.listfileList();
        }
      });
    },
    exportData2() {
      this.loadingChanpoi = true;
      exportwyhc(this.formDataLIstr).then((ok) => {
        if (ok.code == 200) {
          this.loadingChanpoi = false;
          this.checkListt();
        }
      });
    },
    ...
    handleEditClick(value) {
      ...
      this.$router.push({
        path: "/Enforcementxiang/Fieldxiang",
        query: {
          id: value.id,
    ...
    handleCurrentChange(value) {
      ...
      this.formDataLIstr.pageNum = value;
      this.findAll();
    },
    ...
    handleChangeXzq(value) {
      this.formDataLIstr.city = "";
      this.formDataLIstr.county = "";
      if (value.length) {
        const textArr = [];
        ...
        this.formDataLIstr.city = arr.join(",");
        this.formDataLIstr.county = county.join(",");
      }
    },
    //数组转对象
    arrayToObject(arr) {
    ...
    findAll(isSearch) {
      ...
      // }
      if (this.formDataLIstr.snapdate) {
        this.formDataLIstr.snapdate = this.formatDate(
          this.formDataLIstr.snapdate
        );
      }

      const { selectXZQ, valuetime, ...params } = this.formDataLIstr;
      console.log(params, "params");
      console.log(this.formDataLIstr, "this.formDataLIstr");
