<template>
  <div class="container">
    <div class="line"></div>
    <div class="sellect">
      <el-form ref="form" :model="formData11" label-width="55px">
        <div>
          <el-row class="title" type="flex" align="middle">
            ...
            <el-col :span="4" :style="{ marginLeft: '15px' }">
              <el-input
                ...
                v-model.trim="formData11.dkOrTb"
                clearable
          ...
          <el-dropdown @command="dropdownItemClick">
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item
                ...
                v-if="permissions['当前清单列表'] === 0"
                >当前清单</el-dropdown-item
              >
              <el-dropdown-item command="uploadFileList"
                >重点区域土地违法清单</el-dropdown-item
              >
            </el-dropdown-menu>
          </el-dropdown>
        </div>
...
export default {
  ...
  methods: {
    ...
    saveFormData() {
      // 创建formDatahut的副本
      let dataToStore = { ...this.formData11 };
      ...
      localStorage.setItem("formData11", JSON.stringify(dataToStore));
    },
    loadFormData() {
      const savedData = localStorage.getItem("formData11");
      if (savedData) {
        this.formData11 = JSON.parse(savedData);
      }
    },
    // saveFormData() {
    //   localStorage.setItem("formData11", JSON.stringify(this.formData11));
    // },
    // loadFormData() {
    //   const savedData = localStorage.getItem("formData11");
    //   if (savedData) {
    ...
    beforeRouteEnter(to, from, next) {
      next((vm) => {
        if (to.params.savedData) {
          vm.formData11 = to.params.savedData;
        }
      });
    },
    handleDateChange(dates) {
      if (Array.isArray(dates) && dates.length > 0) {
        ...
        this.formData11.startDate = formattedDates[0];
        this.formData11.endDate = formattedDates[1];
      } else {
        this.formData11.startDate = "";
        this.formData11.endDate = "";
      }
    },
    emitGetData(data) {
      // console.log(data);
      this.formData11.sfwttb = data.sfwttb;
      this.formData11.hcjd = data.hcjd;
      this.formData11.pdlx = data.tblx;
      this.formData11.shengjidxjg = data.checkMethod;
      this.formData11.shijidxjg = data.illegalityType;
      this.formData11.qbtbHchpdlx = data.hchlxx;
      this.formData11.qbtbSfnrwfjs = data.deductionType;
      this.formData11.qbtbTbly = data.tbly;
      this.formData11.tbfl = data.tbfl;
      this.formData11.qbtbCancelStatus = data.qbtbCancelstatus;

      this.formData11.hcjd = data.hcjd;
      this.formData11.xmlx = data.xmlx;
      this.formData11.zglsqk = data.zglsqk;
      this.formData11.qtjtqx = data.qtjtqx;
      this.formData11.hfjtqx = data.hfjtqx;
      this.formData11.wfsjyt = data.sjyt;

      const updateFormData = (field, value, condition) => {
        switch (condition) {
          case "3":
            this.formData11[field] = value;
            break;
          case "1":
            this.formData11[`${field}Max`] = value;
            break;
          case "2":
            this.formData11[`${field}Min`] = value;
            break;
          case "4":
            this.formData11[`dydy${field}`] = value;
            break;
          case "5":
            this.formData11[`xydy${field}`] = value;
            break;
          case "6":
            const [min, max] = value.split("~");
            this.formData11[`${field}Max`] = max;
            this.formData11[`${field}Min`] = min;
            break;
        }
      };
      ...
      if (data.freck.condition === "4") {
        this.formData11.dydygdmj = data.freck.value;
      } else if (data.freck.condition === "5") {
        this.formData11.xydygdmj = data.freck.value;
      } else {
        updateFormData("gdmj", data.freck.value, data.freck.condition);
      }

      if (data.Cufreck.condition === "4") {
        this.formData11.dydynydmj = data.Cufreck.value;
      } else if (data.Cufreck.condition === "5") {
        this.formData11.xydynydmj = data.Cufreck.value;
      } else {
    ...
    terDialog() {
      if (this.namerow == "") {
        ...
        this.formData11.remarks = this.namerow;
        let data = { ...this.formData11 };
    ...
    exportData() {
      ...
      if (this.tltlelp == "重点区域土地违法清单") {
        specialcjhhexport(this.formData11).then((ok) => {
          if (ok.code === 200) {
        ...
        special2024cjhh(this.formData11)
          .then((res) => {
            if (res.code === 200) {
              this.loadingqingdan = false;
              this.listfileList();
    ...
    handleCurrentChange(value) {
      this.formData11.pageNum = value;
      this.findAll();
    },
    handleSizeli(value) {
      this.formData11.pageSize = value;
      this.findAll();
    },
    ...
    handleChangeXzq(value) {
      this.formData11.city = "";
      this.formData11.county = "";
      if (value.length) {
        const textArr = [];
        ...
        this.formData11.city = arr.join(",");
        this.formData11.county = county.join(",");
      }
    },
    //数组转对象
    arrayToObject(arr) {
    ...
    findAll(isSearch) {
      ...
      // }
      if (this.formData11.snapdate) {
        this.formData11.snapdate = this.formatDate(this.formData11.snapdate);
      }

      const { selectXZQ, valuetime, ...params } = this.formData11;
