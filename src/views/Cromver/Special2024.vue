<template>
  <div class="container">
    <div class="line"></div>
    <div class="sellect">
      <el-form ref="form" :model="formData10" label-width="55px">
        <div>
          <el-row class="title" type="flex" align="middle">
            ...
            <el-col :span="4" :style="{ marginLeft: '15px' }">
              <el-input
                ...
                v-model.trim="formData10.dkOrTb"
                clearable
          ...
          <el-dropdown @command="dropdownItemClick">
            ...
            <el-dropdown-menu slot="dropdown">
              ...
              <el-dropdown-item command="uploadFileList"
                >疑似违法清单</el-dropdown-item
              >
              <el-dropdown-item command="qualityCheckList"
                >土地乱搭乱建台账清单</el-dropdown-item
              >
            </el-dropdown-menu>
          </el-dropdown>
        </div>
...
export default {
  ...
  methods: {
    ...
    shanliop() {
      ...
      } else if (this.titleko == "生态保护乱搭乱建清单") {
        let ids = this.idrowp.slice();
        fileList02(ids).then((ok) => {
          if (ok.code == 200) {
            this.$message({
      ...
      // if (
      //   this.titleko == "重点区域土地违法清单" ||
      //   this.titleko == "生态保护乱搭乱建清单" ||
      //   this.titleko == "当前清单"
      // ) {
      //   fileList02(ids).then((ok) => {
      //     if (ok.code == 200) {
    ...
    saveFormData() {
      // 创建formDatahut的副本
      let dataToStore = { ...this.formData10 };
      ...
      localStorage.setItem("formData10", JSON.stringify(dataToStore));
    },
    loadFormData() {
      const savedData = localStorage.getItem("formData10");
      if (savedData) {
        this.formData10 = JSON.parse(savedData);
      }
    },
    // saveFormData() {
    //   localStorage.setItem("formData10", JSON.stringify(this.formData10));
    // },
    // loadFormData() {
    //   const savedData = localStorage.getItem("formData10");
    //   if (savedData) {
    ...
    beforeRouteEnter(to, from, next) {
      next((vm) => {
        if (to.params.savedData) {
          vm.formData10 = to.params.savedData;
        }
      });
    },
    handleDateChange(dates) {
      if (Array.isArray(dates) && dates.length > 0) {
        ...
        this.formData10.startDate = formattedDates[0];
        this.formData10.endDate = formattedDates[1];
      } else {
        this.formData10.startDate = "";
        this.formData10.endDate = "";
      }
    },
    emitGetData(data) {
      // console.log(data);
      this.formData10.sfwttb = data.sfwttb;
      this.formData10.hcjd = data.hcjd;
      this.formData10.pdlx = data.tblx;
      this.formData10.shengjidxjg = data.checkMethod;
      this.formData10.shijidxjg = data.illegalityType;
      this.formData10.qbtbHchpdlx = data.hchlxx;
      this.formData10.qbtbSfnrwfjs = data.deductionType;
      this.formData10.qbtbTbly = data.tbly;
      this.formData10.tbfl = data.tbfl;
      this.formData10.qbtbCancelStatus = data.qbtbCancelstatus;

      this.formData10.hcjd = data.hcjd;
      this.formData10.xmlx = data.xmlx;
      this.formData10.zglsqk = data.zglsqk;
      this.formData10.qtjtqx = data.qtjtqx;
      this.formData10.hfjtqx = data.hfjtqx;
      this.formData10.wfsjyt = data.sjyt;

      const updateFormData = (field, value, condition) => {
        switch (condition) {
          case "3":
            this.formData10[field] = value;
            break;
          case "1":
            this.formData10[`${field}Max`] = value;
            break;
          case "2":
            this.formData10[`${field}Min`] = value;
            break;
          case "4":
            this.formData10[`dydy${field}`] = value;
            break;
          case "5":
            this.formData10[`xydy${field}`] = value;
            break;
          case "6":
            const [min, max] = value.split("~");
            this.formData10[`${field}Max`] = max;
            this.formData10[`${field}Min`] = min;
            break;
        }
      };
      ...
      if (data.freck.condition === "4") {
        this.formData10.dydygdmj = data.freck.value;
      } else if (data.freck.condition === "5") {
        this.formData10.xydygdmj = data.freck.value;
      } else {
        updateFormData("gdmj", data.freck.value, data.freck.condition);
      }

      if (data.Cufreck.condition === "4") {
        this.formData10.dydynydmj = data.Cufreck.value;
      } else if (data.Cufreck.condition === "5") {
        this.formData10.xydynydmj = data.Cufreck.value;
      } else {
    ...
    terDialog() {
      if (this.namerow == "") {
        ...
        this.formData10.remarks = this.namerow;
        let data = { ...this.formData10 };
    ...
    grouplistrow() {
      if (this.titleko == "生态保护乱搭乱建清单") {
        this.qualityCheckList();
      } else if (this.titleko == "当前清单") {
        this.listfileList();
      } else if (this.titleko == "重点区域土地违法清单") {
        this.uploadFileList();
      }
    },
    // handleSearch() {
    ...
    exportData() {
      ...
      if (this.titleko == "生态保护乱搭乱建清单") {
        fileListexport(this.formData10).then((ok) => {
          // console.log(ok.code);
          if (ok.code == 200) {
        ...
        special2024sfql(this.formData10)
          .then((ok) => {
            if (ok.code == 200) {
              this.loadingqingdan = false;
              this.listfileList();
        ...
        special202zdqy(this.formData10)
          .then((ok) => {
            if (ok.code == 200) {
              this.loadingqingdan = false;
              this.uploadFileList();
    ...
    handleCurrentChange(value) {
      this.formData10.pageNum = value;
      this.findAll();
    },
    handleSizeli(value) {
      this.formData10.pageSize = value;
      this.findAll();
    },
    ...
    handleChangeXzq(value) {
      this.formData10.city = "";
      this.formData10.county = "";
      if (value.length) {
        const textArr = [];
        ...
        this.formData10.city = arr.join(",");
        this.formData10.county = county.join(",");
      }
    },
    //数组转对象
    arrayToObject(arr) {
    ...
    findAll(isSearch) {
      ...
      // }
      if (this.formData10.snapdate) {
        this.formData10.snapdate = this.formatDate(this.formData10.snapdate);
      }

      const { selectXZQ, valuetime, ...params } = this.formData10;
