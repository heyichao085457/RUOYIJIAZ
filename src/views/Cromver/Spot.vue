<template>
  <div class="container">
    <div class="line"></div>
    <div class="sellect">
      <el-form ref="form" :model="formData13" label-width="55px">
        <div>
          <el-row class="title" type="flex" align="middle">
            ...
            <el-col :span="4" :style="{ marginLeft: '15px' }">
              <el-input
                ...
                v-model.trim="formData13.dkOrTb"
                clearable
...
export default {
  ...
  methods: {
    ...
    saveFormData() {
      localStorage.setItem("formData13", JSON.stringify(this.formData13));
    },
    loadFormData() {
      const savedData = localStorage.getItem("formData13");
      if (savedData) {
        this.formData13 = JSON.parse(savedData);
      }
    },
    ...
    beforeRouteEnter(to, from, next) {
      next((vm) => {
        if (to.params.savedData) {
          vm.formData13 = to.params.savedData;
        }
      });
    },
    handleDateChange(dates) {
      if (Array.isArray(dates) && dates.length > 0) {
        ...
        this.formData13.startDate = formattedDates[0];
        this.formData13.endDate = formattedDates[1];
      } else {
        this.formData13.startDate = "";
        this.formData13.endDate = "";
      }
    },
    emitGetData(data) {
      // console.log(data);
      this.formData13.sfwttb = data.sfwttb;
      this.formData13.hcjd = data.hcjd;
      this.formData13.pdlx = data.tblx;
      this.formData13.shengjidxjg = data.checkMethod;
      this.formData13.shijidxjg = data.illegalityType;
      this.formData13.qbtbHchpdlx = data.hchlxx;
      this.formData13.qbtbSfnrwfjs = data.deductionType;
      this.formData13.qbtbTbly = data.tbly;
      this.formData13.tbfl = data.tbfl;
      this.formData13.qbtbCancelStatus = data.qbtbCancelstatus;

      this.formData13.hcjd = data.hcjd;
      this.formData13.xmlx = data.xmlx;
      this.formData13.zglsqk = data.zglsqk;
      this.formData13.qtjtqx = data.qtjtqx;
      this.formData13.hfjtqx = data.hfjtqx;
      this.formData13.wfsjyt = data.sjyt;

      const updateFormData = (field, value, condition) => {
        switch (condition) {
          case "3":
            this.formData13[field] = value;
            break;
          case "1":
            this.formData13[`${field}Max`] = value;
            break;
          case "2":
            this.formData13[`${field}Min`] = value;
            break;
          case "4":
            this.formData13[`dydy${field}`] = value;
            break;
          case "5":
            this.formData13[`xydy${field}`] = value;
            break;
          case "6":
            const [min, max] = value.split("~");
            this.formData13[`${field}Max`] = max;
            this.formData13[`${field}Min`] = min;
            break;
        }
      };
      ...
      if (data.freck.condition === "4") {
        this.formData13.dydygdmj = data.freck.value;
      } else if (data.freck.condition === "5") {
        this.formData13.xydygdmj = data.freck.value;
      } else {
        updateFormData("gdmj", data.freck.value, data.freck.condition);
      }

      if (data.Cufreck.condition === "4") {
        this.formData13.dydynydmj = data.Cufreck.value;
      } else if (data.Cufreck.condition === "5") {
        this.formData13.xydynydmj = data.Cufreck.value;
      } else {
    ...
    terDialog() {
      if (this.namerow == "") {
        ...
        this.formData13.remarks = this.namerow;
        let data = { ...this.formData13 };
    ...
    exportData() {
      ...
      special2024kctb(this.formData13)
        .then((res) => {
          if (res.code === 200) {
    ...
    handleCurrentChange(value) {
      this.formData13.pageNum = value;
      this.findAll();
    },
    handleSizeli(value) {
      this.formData13.pageSize = value;
      this.findAll();
    },
    ...
    handleChangeXzq(value) {
      this.formData13.city = "";
      this.formData13.county = "";
      if (value.length) {
        const textArr = [];
        ...
        this.formData13.city = arr.join(",");
        this.formData13.county = county.join(",");
      }
    },
    //数组转对象
    arrayToObject(arr) {
    ...
    findAll(isSearch) {
      ...
      // }
      if (this.formData13.snapdate) {
        this.formData13.snapdate = this.formatDate(this.formData13.snapdate);
      }

      const { selectXZQ, valuetime, ...params } = this.formData13;
