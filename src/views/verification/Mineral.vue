<template>
  <div class="container">
    <div class="line"></div>
    <div class="sellect">
      <el-form ref="form" :model="formDatatbfk" label-width="55px">
        <div>
          <el-row class="title" type="flex" align="middle">
            <el-col :span="4">
              <el-form-item label="行政区">
                <el-cascader
                  ref="cascader"
                  clearable
                  v-model="formDatatbfk.selectXZQ"
                  :options="xzq"
                  :style="{ width: '100%' }"
                  :props="{
                    value: 'value',
            ...
            <el-col :span="5">
              <el-date-picker
                v-model="formDatatbfk.valuetime"
                type="daterange"
                range-separator="至"
            ...
            <el-col :span="4" :style="{ marginLeft: '15px' }">
              <el-input
                ...
                v-model.trim="formDatatbfk.dkOrTb"
                clearable
              ...
              <div>
                ...
                >
                  ...
                  >
                    <el-row>
                      <el-col :span="24">
                        <el-form-item label="合法性判定">
                          <el-select
                            v-model="formDatatbfk.hfxpd"
                            clearable
                            placeholder="合法性判定"
                          >
                            <el-option
                      ...
                      <el-col :span="24">
                        <el-form-item label="是否在秦岭范围">
                          <el-select
                            v-model="formDatatbfk.sfqlfw"
                            clearable
                            placeholder="是否在秦岭范围"
                          >
                            <el-option
                      ...
                      <el-col :span="24">
                        <el-form-item label="是否在长江范围">
                          <el-select
                            v-model="formDatatbfk.sfcjqy"
                            clearable
                            placeholder="是否在长江范围"
                          >
                            <el-option
    ...
    <div class="table-container">
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <el-tooltip :content="scope.row.kqwz" placement="top" effect="dark">
              <div class="address-cell">
                {{ scope.row.kqwz }}
              </div>
            </el-tooltip>
    ...
    <div class="flexlist">
      <div style="width: 50%; height: 30px">
        <el-row class="title" type="flex" align="middle">
          <div :style="{ marginLeft: '10px' }"  v-if="permissions['当前清单列表'] === 0">
...
export default {
  ...
  methods: {
    ...
    Dropdownfxw() {
      DropdownBoxKctb().then((ok) => {
        ok.data.HFXPD.forEach((ele) => {
          this.options.push({ value: ele, label: ele });
        });
        ok.data.SFQLFW.forEach((ele) => {
          this.options1.push({ value: ele, label: ele });
        });
        ok.data.SFCJFW.forEach((ele) => {
    ...
    findAll(reload) {
      console.log("查询:", reload);
    },
    saveFormData() {
      localStorage.setItem("formDatatbfk", JSON.stringify(this.formDatatbfk));
    },
    loadFormData() {
      const savedData = localStorage.getItem("formDatatbfk");
      if (savedData) {
        this.formDatatbfk = JSON.parse(savedData);
      }
    },
    ...
    beforeRouteEnter(to, from, next) {
      next((vm) => {
        if (to.params.savedData) {
          vm.formDatatbfk = to.params.savedData;
        }
      });
    },
    handleDateChange(dates) {
      if (Array.isArray(dates) && dates.length > 0) {
        ...
        this.formDatatbfk.startDate = formattedDates[0];
        this.formDatatbfk.endDate = formattedDates[1];
      } else {
        this.formDatatbfk.startDate = "";
        this.formDatatbfk.endDate = "";
      }
    },
    emitGetData(data) {
      // console.log(data);
      this.formDatatbfk.sfwttb = data.sfwttb;
      this.formDatatbfk.hcjd = data.hcjd;
      this.formDatatbfk.pdlx = data.tblx;
      this.formDatatbfk.shengjidxjg = data.checkMethod;
      this.formDatatbfk.shijidxjg = data.illegalityType;
      this.formDatatbfk.qbtbHchpdlx = data.hchlxx;
      this.formDatatbfk.qbtbSfnrwfjs = data.deductionType;
      this.formDatatbfk.qbtbTbly = data.tbly;
      this.formDatatbfk.tbfl = data.tbfl;
      this.formDatatbfk.qbtbCancelStatus = data.qbtbCancelstatus;

      this.formDatatbfk.hcjd = data.hcjd;
      this.formDatatbfk.xmlx = data.xmlx;
      this.formDatatbfk.zglsqk = data.zglsqk;
      this.formDatatbfk.qtjtqx = data.qtjtqx;
      this.formDatatbfk.hfjtqx = data.hfjtqx;
      this.formDatatbfk.wfsjyt = data.sjyt;

      const updateFormData = (field, value, condition) => {
        switch (condition) {
          case "3":
            this.formDatatbfk[field] = value;
            break;
          case "1":
            this.formDatatbfk[`${field}Max`] = value;
            break;
          case "2":
            this.formDatatbfk[`${field}Min`] = value;
            break;
          case "4":
            this.formDatatbfk[`dydy${field}`] = value;
            break;
          case "5":
            this.formDatatbfk[`xydy${field}`] = value;
            break;
          case "6":
            const [min, max] = value.split("~");
            this.formDatatbfk[`${field}Max`] = max;
            this.formDatatbfk[`${field}Min`] = min;
            break;
        }
      };
      ...
      if (data.freck.condition === "4") {
        this.formDatatbfk.dydygdmj = data.freck.value;
      } else if (data.freck.condition === "5") {
        this.formDatatbfk.xydygdmj = data.freck.value;
      } else {
        updateFormData("gdmj", data.freck.value, data.freck.condition);
      }

      if (data.Cufreck.condition === "4") {
        this.formDatatbfk.dydynydmj = data.Cufreck.value;
      } else if (data.Cufreck.condition === "5") {
        this.formDatatbfk.xydynydmj = data.Cufreck.value;
      } else {
    ...
    terDialog() {
      if (this.namerow == "") {
        ...
        this.formDatatbfk.remarks = this.namerow;
        let data = { ...this.formDatatbfk };
    ...
    exportData() {
      ...
      specialsfql2023kctb(this.formDatatbfk)
        .then((res) => {
          if (res.code === 200) {
    ...
    handleCurrentChange(value) {
      this.formDatatbfk.pageNum = value;
      this.findAll();
    },
    handleSizeli(value) {
      this.formDatatbfk.pageSize = value;
      this.findAll();
    },
    ...
    handleChangeXzq(value) {
      this.formDatatbfk.city = "";
      this.formDatatbfk.county = "";
      if (value.length) {
        const textArr = [];
        ...
        this.formDatatbfk.city = arr.join(",");
        this.formDatatbfk.county = county.join(",");
      }
    },
    //数组转对象
    arrayToObject(arr) {
    ...
    findAll(isSearch) {
      ...
      // }
      if (this.formDatatbfk.snapdate) {
        this.formDatatbfk.snapdate = this.formatDate(
          this.formDatatbfk.snapdate
        );
      }

      const { selectXZQ, valuetime, ...params } = this.formDatatbfk;
