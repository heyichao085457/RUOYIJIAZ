<template>
  <div class="container">
    <div class="line"></div>
    <div class="sellect">
      <el-form ref="form" :model="formDatajut" label-width="55px">
        <div>
          <el-row class="title" type="flex" align="middle">
            ...
            <el-col :span="4" :style="{ marginLeft: '15px' }">
              <el-input
                ...
                v-model.trim="formDatajut.dkOrTb"
                clearable
...
export default {
  ...
  methods: {
    ...
    saveFormData() {
      // 创建formDatahut的副本
      let dataToStore = { ...this.formDatajut };
      ...
      localStorage.setItem("formDatajut", JSON.stringify(dataToStore));
    },
    loadFormData() {
      const savedData = localStorage.getItem("formDatajut");
      if (savedData) {
        this.formDatajut = JSON.parse(savedData);
      }
    },
    // saveFormData() {
    //   localStorage.setItem("formDatajut", JSON.stringify(this.formDatajut));
    // },
    // loadFormData() {
    //   const savedData = localStorage.getItem("formDatajut");
    //   if (savedData) {
    ...
    beforeRouteEnter(to, from, next) {
      next((vm) => {
        if (to.params.savedData) {
          vm.formDatajut = to.params.savedData;
        }
      });
    },
    handleDateChange(dates) {
      if (Array.isArray(dates) && dates.length > 0) {
        ...
        this.formDatajut.startDate = formattedDates[0];
        this.formDatajut.endDate = formattedDates[1];
      } else {
        this.formDatajut.startDate = "";
        this.formDatajut.endDate = "";
      }
    },
    emitGetData(data) {
      // console.log(data);
      this.formDatajut.sfwttb = data.sfwttb;
      this.formDatajut.hcjd = data.hcjd;
      this.formDatajut.pdlx = data.tblx;
      this.formDatajut.shengjidxjg = data.checkMethod;
      this.formDatajut.shijidxjg = data.illegalityType;
      this.formDatajut.qbtbHchpdlx = data.hchlxx;
      this.formDatajut.qbtbSfnrwfjs = data.deductionType;
      this.formDatajut.qbtbTbly = data.tbly;
      this.formDatajut.tbfl = data.tbfl;
      this.formDatajut.qbtbCancelStatus = data.qbtbCancelstatus;

      this.formDatajut.hcjd = data.hcjd;
      this.formDatajut.xmlx = data.xmlx;
      this.formDatajut.zglsqk = data.zglsqk;
      this.formDatajut.qtjtqx = data.qtjtqx;
      this.formDatajut.hfjtqx = data.hfjtqx;
      this.formDatajut.wfsjyt = data.sjyt;

      const updateFormData = (field, value, condition) => {
        switch (condition) {
          case "3":
            this.formDatajut[field] = value;
            break;
          case "1":
            this.formDatajut[`${field}Max`] = value;
            break;
          case "2":
            this.formDatajut[`${field}Min`] = value;
            break;
          case "4":
            this.formDatajut[`dydy${field}`] = value;
            break;
          case "5":
            this.formDatajut[`xydy${field}`] = value;
            break;
          case "6":
            const [min, max] = value.split("~");
            this.formDatajut[`${field}Max`] = max;
            this.formDatajut[`${field}Min`] = min;
            break;
        }
      };
      ...
      if (data.freck.condition === "4") {
        this.formDatajut.dydygdmj = data.freck.value;
      } else if (data.freck.condition === "5") {
        this.formDatajut.xydygdmj = data.freck.value;
      } else {
        updateFormData("gdmj", data.freck.value, data.freck.condition);
      }

      if (data.Cufreck.condition === "4") {
        this.formDatajut.dydynydmj = data.Cufreck.value;
      } else if (data.Cufreck.condition === "5") {
        this.formDatajut.xydynydmj = data.Cufreck.value;
      } else {
    ...
    terDialog() {
      if (this.namerow == "") {
        ...
        this.formDatajut.remarks = this.namerow;
        let data = { ...this.formDatajut };
    ...
    handleCurrentChange(value) {
      this.formDatajut.pageNum = value;
      this.findAll();
    },
    handleSizeli(value) {
      this.formDatajut.pageSize = value;
      this.findAll();
    },
    ...
    handleChangeXzq(value) {
      this.formDatajut.city = "";
      this.formDatajut.county = "";
      if (value.length) {
        const textArr = [];
        ...
        this.formDatajut.city = arr.join(",");
        this.formDatajut.county = county.join(",");
      }
    },
    //数组转对象
    arrayToObject(arr) {
    ...
    findAll(isSearch) {
      ...
      // }
      if (this.formDatajut.snapdate) {
        this.formDatajut.snapdate = this.formatDate(this.formDatajut.snapdate);
      }

      const { selectXZQ, valuetime, ...params } = this.formDatajut;
