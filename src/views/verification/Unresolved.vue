<template>
  <div class="container">
          ...
          <el-row class="title" type="flex" align="middle">
            ...
            <el-col :span="4" :style="{ marginLeft: '15px' }">
              <el-input
                size="medium"
                placeholder="请输入地块编号"
                v-model.trim="formDatahut.landCode"
                clearable
    ...
    <div class="table-container">
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <el-tooltip :content="scope.row.tdzl" placement="top" effect="dark">
              <div class="address-cell">
                {{ scope.row.tdzl }}
              </div>
            </el-tooltip>
        ...
        >
          <template slot-scope="scope">
            <el-tooltip :content="scope.row.sjyt" placement="top" effect="dark">
              <div class="address-cell">
                {{ scope.row.sjyt }}
              </div>
            </el-tooltip>
        ...
        >
          <template slot-scope="scope">
            <el-tooltip
              :content="scope.row.sfbzxajgcjs"
              placement="top"
              effect="dark"
            >
              <div class="address-cell">
                {{ scope.row.sfbzxajgcjs }}
              </div>
            </el-tooltip>
          </template>
        </el-table-column>
        ...
        >
          <template slot-scope="scope">
            <el-tooltip :content="scope.row.sfja" placement="top" effect="dark">
              <div class="address-cell">
                {{ scope.row.sfja }}
              </div>
            </el-tooltip>
        ...
        >
          <template slot-scope="scope">
            <el-tooltip
              :content="scope.row.wfydsjhyzgbm"
              placement="top"
              effect="dark"
            >
              <div class="address-cell">
                {{ scope.row.wfydsjhyzgbm }}
              </div>
            </el-tooltip>
          </template>
        </el-table-column>
        ...
        >
          <template slot-scope="scope">
            <el-tooltip :content="scope.row.sfnr" placement="top" effect="dark">
              <div class="address-cell">
                {{ scope.row.sfnr }}
              </div>
            </el-tooltip>
        ...
        >
          <template slot-scope="scope">
            <el-tooltip :content="scope.row.bh" placement="top" effect="dark">
              <div class="address-cell">
                {{ scope.row.bh }}
              </div>
            </el-tooltip>
        ...
        >
          <template slot-scope="scope">
            <el-tooltip
              :content="scope.row.sffhsqsxhdcg"
              placement="top"
              effect="dark"
            >
              <div class="address-cell">
                {{ scope.row.sffhsqsxhdcg }}
              </div>
            </el-tooltip>
          </template>
        </el-table-column>
        ...
        >
          <template slot-scope="scope">
            <el-tooltip
              :content="scope.row.sfynrcpkffa"
              placement="top"
              effect="dark"
            >
              <div class="address-cell">
                {{ scope.row.sfynrcpkffa }}
              </div>
            </el-tooltip>
          </template>
        </el-table-column>
        ...
        >
          <template slot-scope="scope">
            <el-tooltip :content="scope.row.year" placement="top" effect="dark">
              <div class="address-cell">
                {{ scope.row.year }}
              </div>
            </el-tooltip>
        ...
        >
          <template slot-scope="scope">
            <el-tooltip
              :content="scope.row.import_batch"
              placement="top"
              effect="dark"
            >
              <div class="address-cell">
                {{ scope.row.import_batch }}
              </div>
            </el-tooltip>
          </template>
        </el-table-column>
    ...
    <div class="flexlist">
      <div style="width: 50%; height: 30px">
        <el-row class="title" type="flex" align="middle">
          ...
          <div
            :style="{ marginLeft: '10px' }"
            v-if="permissions['生成未整改图斑数据'] === 0"
          >
            <el-button
              size="mini"
              type="primary"
              @click="scshqkbg('生成未整改图斑数据')"
              >生成未整改图斑数据</el-button
            >
          </div>
          <div :style="{ marginLeft: '10px' }">
            <el-button
              size="mini"
              type="primary"
              @click="scshqkbg('生成违法占耕地整改情况报告')"
              >生成违法占耕地整改情况报告</el-button
            >
          </div>
        </el-row>
      </div>
      ...
      <div class="dialoglbodylo">
        ...
        >
          ...
          <el-button type="primary" @click="qualityinspection"
            >模板下载</el-button
          >
          <el-button type="primary" @click="beforeUpload">上传</el-button>
        </el-dialog>
  ...
  <script>
import screen from "../screen/screener.vue";
...
export default {
  ...
  methods: {
    ...
    shhqkExpo(name) {
      ...
      if (name == "生成未整改图斑数据") {
        ...
        gdbhb().then((ok) => {
          this.urlpo = ok.msg;
          let url = ok.msg;
          const parts = url.split(":8084/")[1];
          if (ok.code == 200) {
            this.loading = false;
            this.urlList = `${baseUrl}/${parts}`;
          }
        });
      } else if (name == "生成违法占耕地整改情况报告") {
        this.docx = true;
        rectification().then((ok) => {
          this.urlpo = ok.msg;
          let url = ok.msg;
          const parts = url.split(":8084/")[1];
          if (ok.code == 200) {
    ...
    enterAs(data) {
      // console.log(data);
      this.formDatahut.sjyt = data.sjyt;
      this.formDatahut.sfbzxajgcjs = data.sfbzxajgcjs;
      this.formDatahut.sffzyq = data.sffzyq;
      this.formDatahut.sfmsgc = data.sfmsgc;
      this.formDatahut.sflacc = data.sflacc;
      this.formDatahut.sfja = data.sfja;
      this.formDatahut.sfzglsdw = data.sfzglsdw;
      this.formDatahut.sfnr = data.sfnr;
      this.formDatahut.sfjbbptj = data.sfjbbptj;
      this.formDatahut.sffhsqsxhdcg = data.sffhsqsxhdcg;
      this.formDatahut.sfynrcpkffa = data.sfynrcpkffa;
      this.formDatahut.nccfg = data.nccfg;
      this.formDatahut.year = data.year;
    ...
    exportData() {
      ...
      wzgwttbexport(this.formDatahut)
        .then((res) => {
          if (res.code === 200) {
    ...
    handleEditClick(value) {
      this.$router.push({
        path: "/Unresolvedxiang/Unresolvedxiang",
        query: {
          id: value.id,
