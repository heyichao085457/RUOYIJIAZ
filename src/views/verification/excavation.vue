<template>
  <div class="container">
    <div class="line"></div>
    <div class="sellect">
      <el-form ref="form" :model="formData202301" label-width="55px">
        <div>
          <el-row class="title" type="flex" align="middle">
            <el-col :span="4">
              <el-form-item label="行政区">
                <el-cascader
                  ref="cascader"
                  clearable
                  v-model="formData202301.selectXZQ"
                  :options="xzq"
                  :style="{ width: '100%' }"
                  :props="{
                    value: 'value',
            ...
            <el-col :span="5">
              <el-date-picker
                v-model="formData202301.valuetime"
                type="daterange"
                range-separator="至"
            ...
            <el-col :span="4" :style="{ marginLeft: '15px' }">
              <el-input
                size="medium"
                placeholder="请输入图斑编号"
                v-model.trim="formData202301.dkOrTb"
                clearable
              ...
              <div>
                ...
                >
                  ...
                  >
                    <el-row>
                      <el-col :span="24">
                        <el-form-item label="违法行业">
                          <el-select
                            v-model="formData202301.wfhy"
                            clearable
                            placeholder="违法行业"
                          >
                            <el-option
...
import MyCustomInput from "../freckleInfractionFarmlandMin/freckleInfr.vue";
import {
  wscs2023,
  options,
  areas,
...
export default {
  ...
  methods: {
    ...
    Dropdownfxw() {
      DropdownBoxWscs().then((ok) => {
        ok.data.WFHY.forEach((ele) => {
          this.options.push({ value: ele, label: ele });
        });
        ok.data.WFZT.forEach((ele) => {
          this.options1.push({ value: ele, label: ele });
        });
      });
    },
    clearFormData() {
      this.formData202301.tbmjMin = "";
      this.formData202301.tbmjMax = "";
      this.formData202301.tbmj = "";
      this.formData202301.dydytbmj = "";
      this.formData202301.xydytbmj = "";
    },
    onSubprimary() {
      this.clearFormData();
      switch (this.customValue.condition) {
        case "1":
          this.formData202301.tbmjMax = this.customValue.value;
          break;
        case "2":
          this.formData202301.tbmjMin = this.customValue.value;
          break;
        case "3":
          this.formData202301.tbmj = this.customValue.value;
          break;
        case "4":
          this.formData202301.dydytbmj = this.customValue.value;
          break;
        case "5":
          this.formData202301.xydytbmj = this.customValue.value;
          break;
        case "6":
          if (this.customValue.value && this.customValue.value.includes("~")) {
            const parts = this.customValue.value.split("~");
            this.formData202301.tbmjMin = parts[0].trim();
            this.formData202301.tbmjMax = parts[1].trim();
          }
          break;
      }
      this.findAll();
    ...
    saveFormData() {
      // 创建formDatahut的副本
      let dataToStore = { ...this.formData202301 };
      ...
      localStorage.setItem("formData202301", JSON.stringify(dataToStore));
    },
    loadFormData() {
      const savedData = localStorage.getItem("formData202301");
      if (savedData) {
        this.formData202301 = JSON.parse(savedData);
      }
    },
    // saveFormData() {
    //   localStorage.setItem("formData202301", JSON.stringify(this.formData202301));
    // },
    // loadFormData() {
    //   const savedData = localStorage.getItem("formData202301");
    //   if (savedData) {
    ...
    beforeRouteEnter(to, from, next) {
      next((vm) => {
        if (to.params.savedData) {
          vm.formData202301 = to.params.savedData;
        }
      });
    },
    handleDateChange(dates) {
      if (Array.isArray(dates) && dates.length > 0) {
        ...
        this.formData202301.startDate = formattedDates[0];
        this.formData202301.endDate = formattedDates[1];
      } else {
        this.formData202301.startDate = "";
        this.formData202301.endDate = "";
      }
    },
    emitGetData(data) {
      // console.log(data);
      this.formData202301.sfwttb = data.sfwttb;
      this.formData202301.hcjd = data.hcjd;
      this.formData202301.pdlx = data.tblx;
      this.formData202301.shengjidxjg = data.checkMethod;
      this.formData202301.shijidxjg = data.illegalityType;
      this.formData202301.qbtbHchpdlx = data.hchlxx;
      this.formData202301.qbtbSfnrwfjs = data.deductionType;
      this.formData202301.qbtbTbly = data.tbly;
      this.formData202301.tbfl = data.tbfl;
      this.formData202301.qbtbCancelStatus = data.qbtbCancelstatus;

      this.formData202301.hcjd = data.hcjd;
      this.formData202301.xmlx = data.xmlx;
      this.formData202301.zglsqk = data.zglsqk;
      this.formData202301.qtjtqx = data.qtjtqx;
      this.formData202301.hfjtqx = data.hfjtqx;
      this.formData202301.wfsjyt = data.sjyt;

      const updateFormData = (field, value, condition) => {
        switch (condition) {
          case "3":
            this.formData202301[field] = value;
            break;
          case "1":
            this.formData202301[`${field}Max`] = value;
            break;
          case "2":
            this.formData202301[`${field}Min`] = value;
            break;
          case "4":
            this.formData202301[`dydy${field}`] = value;
            break;
          case "5":
            this.formData202301[`xydy${field}`] = value;
            break;
          case "6":
            const [min, max] = value.split("~");
            this.formData202301[`${field}Max`] = max;
            this.formData202301[`${field}Min`] = min;
            break;
        }
      };
      ...
      if (data.freck.condition === "4") {
        this.formData202301.dydygdmj = data.freck.value;
      } else if (data.freck.condition === "5") {
        this.formData202301.xydygdmj = data.freck.value;
      } else {
        updateFormData("gdmj", data.freck.value, data.freck.condition);
      }

      if (data.Cufreck.condition === "4") {
        this.formData202301.dydynydmj = data.Cufreck.value;
      } else if (data.Cufreck.condition === "5") {
        this.formData202301.xydynydmj = data.Cufreck.value;
      } else {
    ...
    terDialog() {
      if (this.namerow == "") {
        ...
        this.formData202301.remarks = this.namerow;
        let data = { ...this.formData202301 };
    ...
    exportData() {
      ...
      specialsfql2023wscss(this.formData202301)
        .then((res) => {
          if (res.code === 200) {
    ...
    handleCurrentChange(value) {
      this.formData202301.pageNum = value;
      this.findAll();
    },
    handleSizeli(value) {
      this.formData202301.pageSize = value;
      this.findAll();
    },
    ...
    handleChangeXzq(value) {
      this.formData202301.city = "";
      this.formData202301.county = "";
      if (value.length) {
        const textArr = [];
        ...
        this.formData202301.city = arr.join(",");
        this.formData202301.county = county.join(",");
      }
    },
    //数组转对象
    arrayToObject(arr) {
    ...
    findAll(isSearch) {
      ...
      // }
      if (this.formData202301.snapdate) {
        this.formData202301.snapdate = this.formatDate(
          this.formData202301.snapdate
        );
      }

      const { selectXZQ, valuetime, ...params } = this.formData202301;
      wscs2023(params)
        .then((res) => {
          if (res.code == 200) {
            load.close();
