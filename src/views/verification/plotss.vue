<template>
  <div class="container">
    <div class="line"></div>
    <div class="sellect">
      <el-form ref="form" :model="formData02" label-width="55px">
        <div>
          <el-row class="title" type="flex" align="middle">
            ...
            <el-col :span="4" :style="{ marginLeft: '15px' }">
              <el-input
                ...
                v-model.trim="formData02.dkOrTb"
                clearable
      ...
      <div style="width: 50%; height: 30px">
        <el-row class="title" type="flex" align="middle">
          <div
            ...
            v-if="permissions['生成县级填报情况报告'] === 0"
          >
            <el-button type="primary" size="mini" @click="scshqkbg"
...
export default {
  ...
  methods: {
    ...
    saveFormData() {
      // 创建formDatahut的副本
      let dataToStore = { ...this.formData02 };
      ...
      localStorage.setItem("formData02", JSON.stringify(dataToStore));
    },
    loadFormData() {
      const savedData = localStorage.getItem("formData02");
      if (savedData) {
        this.formData02 = JSON.parse(savedData);
      }
    },
    // saveFormData() {
    //   localStorage.setItem("formData02", JSON.stringify(this.formData02));
    // },
    // loadFormData() {
    //   const savedData = localStorage.getItem("formData02");
    //   if (savedData) {
    ...
    beforeRouteEnter(to, from, next) {
      next((vm) => {
        if (to.params.savedData) {
          vm.formData02 = to.params.savedData;
        }
      });
    },
    handleDateChange(dates) {
      if (Array.isArray(dates) && dates.length > 0) {
        ...
        this.formData02.startDate = formattedDates[0];
        this.formData02.endDate = formattedDates[1];
      } else {
        this.formData02.startDate = "";
        this.formData02.endDate = "";
      }
    },
    emitGetData(data) {
      // console.log(data);
      this.formData02.sfwttb = data.sfwttb;
      this.formData02.hcjd = data.hcjd;
      this.formData02.pdlx = data.tblx;
      this.formData02.shengjidxjg = data.checkMethod;
      this.formData02.shijidxjg = data.illegalityType;
      this.formData02.qbtbHchpdlx = data.hchlxx;
      this.formData02.qbtbSfnrwfjs = data.deductionType;
      this.formData02.qbtbTbly = data.tbly;
      this.formData02.tbfl = data.tbfl;
      this.formData02.qbtbCancelStatus = data.qbtbCancelstatus;
      this.formData.hcjd = data.hcjd;
      this.formData.xmlx = data.xmlx;
      this.formData.zglsqk = data.zglsqk;
      this.formData.qtjtqx = data.qtjtqx;
      this.formData.hfjtqx = data.hfjtqx;
      this.formData.wfsjyt = data.sjyt;
      const updateFormData = (field, value, condition) => {
        switch (condition) {
          case "3":
            this.formData02[field] = value;
            break;
          case "1":
            this.formData02[`${field}Max`] = value;
            break;
          case "2":
            this.formData02[`${field}Min`] = value;
            break;
          case "4":
            this.formData02[`dydy${field}`] = value;
            break;
          case "5":
            this.formData02[`xydy${field}`] = value;
            break;
          case "6":
            const [min, max] = value.split("~");
            this.formData02[`${field}Max`] = max;
            this.formData02[`${field}Min`] = min;
            break;
        }
      };
      ...
      if (data.freck.condition === "4") {
        this.formData02.dydygdmj = data.freck.value;
      } else if (data.freck.condition === "5") {
        this.formData02.xydygdmj = data.freck.value;
      } else {
        updateFormData("gdmj", data.freck.value, data.freck.condition);
      }

      if (data.Cufreck.condition === "4") {
        this.formData02.dydynydmj = data.Cufreck.value;
      } else if (data.Cufreck.condition === "5") {
        this.formData02.xydynydmj = data.Cufreck.value;
      } else {
    ...
    terDialog() {
      this.formData02.remarks = this.namerow;
      let data = { ...this.formData02 };
    ...
    exportData() {
      ...
      let exportRhtz2024 = {
        city: this.formData02.city,
        county: this.formData02.county,
        ssqx: this.formData02.ssqx,
        cancelStatus: this.formData02.cancelStatus,
        checkDetermineResult: this.formData02.checkDetermineResult,
        startDate: this.formData02.startDate,
        endDate: this.formData02.endDate,
        checkJobName: this.formData02.checkJobName,
        dkOrTb: this.formData02.dkOrTb,
        snapdate: this.formData02.snapdate,
        checkMethod: this.formData02.checkMethod,
        sfwttb: this.formData02.sfwttb,
        pdlx: this.formData02.pdlx,
    ...
    handleEditClick(value) {
      ...
      this.$router.push({
        path: "/Enforcementxiang/Allplotsxiang2023",
        query: {
          // id: value.id,
    ...
    handleChangeXzq(value) {
      this.formData02.city = "";
      this.formData02.county = "";
      if (value.length) {
        const textArr = [];
        ...
        this.formData02.city = arr.join(",");
        this.formData02.county = county.join(",");
      }
    },
    //数组转对象
    arrayToObject(arr) {
    ...
    findAll(isSearch) {
      ...
      // }
      if (this.formData02.snapdate) {
        this.formData02.snapdate = this.formatDate(this.formData02.snapdate);
      }

      const { selectXZQ, valuetime, ...params } = this.formData02;
      qbtbListqbtb2023(params)
        .then((res) => {
