<template>
  <div class="container">
    <div class="line"></div>
    <div class="sellect">
      <el-form ref="form" :model="formData55" label-width="55px">
        <div>
          <el-row class="title" type="flex" align="middle">
            ...
            <el-col :span="4" :style="{ marginLeft: '15px' }">
              <el-input
                ...
                v-model.trim="formData55.dkOrTb"
                clearable
...
export default {
  ...
  methods: {
    handleSizeli(value) {
      this.formData55.pageSize = value;
      this.findAll();
    },
    handleSelece(val) {
      // console.log(val, "val");
    ...
    saveFormData() {
      // 创建formDatahut的副本
      let dataToStore = { ...this.formData55 };
      ...
      localStorage.setItem("formData55", JSON.stringify(dataToStore));
    },
    loadFormData() {
      const savedData = localStorage.getItem("formData55");
      if (savedData) {
        this.formData55 = JSON.parse(savedData);
      }
    },
    // saveFormData() {
    //   localStorage.setItem("formData55", JSON.stringify(this.formData55));
    // },
    // loadFormData() {
    //   const savedData = localStorage.getItem("formData55");
    //   if (savedData) {
    ...
    beforeRouteEnter(to, from, next) {
      next((vm) => {
        if (to.params.savedData) {
          vm.formData55 = to.params.savedData;
        }
      });
    },
    handleDateChange(dates) {
      if (Array.isArray(dates) && dates.length > 0) {
        ...
        this.formData55.startDate = formattedDates[0];
        this.formData55.endDate = formattedDates[1];
      } else {
        this.formData55.startDate = "";
        this.formData55.endDate = "";
      }
    },
    emitGetData(data) {
      // console.log(data);
      this.formData55.tblx = data.tblx;
      this.formData55.tbhcjd = data.tbhcjd;
      this.formData55.hczt = data.hczt;
      this.formData55.jzzt = data.jzzt;
      this.formData55.hchlx = data.hchlxx;
      this.formData55.wfzl = data.wfzl;
      this.formData55.zzyj = data.zzyj;
      this.formData55.wyhcCancelStatus = data.wyhcCancelStatus;
      this.formData55.wyhcSfnrwfjs = data.wyhcSfnrwfjs;
    ...
    terDialog() {
      this.formData55.remarks = this.namerow;
      let data = { ...this.formData55 };
    ...
    exportData() {
      ...
      // exportBylandCode(params).then((ok) => {
      //   if (ok.code == 200) {
      ...
      let freckleCodes = this.selectedRows
        .map((row) => row.freckleCode)
        .join(",");
      let params = {
        landCode: landCodes,
        freckleCode: freckleCodes,
      };
      exportBylandCode2023(params).then((ok) => {
        if (ok.code == 200) {
    ...
    handleEditClick(value) {
      this.$router.push({
        path: "/Enforcementxiang/wyhc2023xiang",
        query: {
          // id: value.id,
    ...
    handleChangeXzq(value) {
      this.formData55.city = "";
      this.formData55.county = "";
      if (value.length) {
        const textArr = [];
        ...
        this.formData55.city = arr.join(",");
        this.formData55.county = county.join(",");
      }
    },
    //数组转对象
    arrayToObject(arr) {
    ...
    findAll(isSearch) {
      ...
      // }
      if (this.formData55.snapdate) {
        this.formData55.snapdate = this.formatDate(this.formData55.snapdate);
      }

      const { selectXZQ, valuetime, ...params } = this.formData55;
