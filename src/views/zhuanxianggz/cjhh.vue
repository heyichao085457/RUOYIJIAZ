<template>
  <div class="container">
    <div class="line"></div>
    <div class="sellect">
      <el-form ref="form" :model="formDataDa" label-width="55px">
        <div>
          <el-row class="title" type="flex" align="middle">
            ...
            <el-col :span="4" :style="{ marginLeft: '15px' }">
              <el-input
                ...
                v-model.trim="formDataDa.dkOrTb"
                clearable
...
export default {
  ...
  methods: {
    ...
    saveFormData() {
      localStorage.setItem("formDataDa", JSON.stringify(this.formDataDa));
    },
    loadFormData() {
      const savedData = localStorage.getItem("formDataDa");
      if (savedData) {
        this.formDataDa = JSON.parse(savedData);
      }
    },
    ...
    beforeRouteEnter(to, from, next) {
      next((vm) => {
        if (to.params.savedData) {
          vm.formDataDa = to.params.savedData;
        }
      });
    },
    handleDateChange(dates) {
      if (Array.isArray(dates) && dates.length > 0) {
        ...
        this.formDataDa.startDate = formattedDates[0];
        this.formDataDa.endDate = formattedDates[1];
      } else {
        this.formDataDa.startDate = "";
        this.formDataDa.endDate = "";
      }
    },
    emitGetData(data) {
      // console.log(data);
      this.formDataDa.sfwttb = data.sfwttb;
      this.formDataDa.hcjd = data.hcjd;
      this.formDataDa.pdlx = data.tblx;
      this.formDataDa.shengjidxjg = data.checkMethod;
      this.formDataDa.shijidxjg = data.illegalityType;
      this.formDataDa.qbtbHchpdlx = data.hchlxx;
      this.formDataDa.qbtbSfnrwfjs = data.deductionType;
      this.formDataDa.qbtbTbly = data.tbly;
      this.formDataDa.tbfl = data.tbfl;
      this.formDataDa.qbtbCancelStatus = data.qbtbCancelstatus;

      this.formDataDa.hcjd = data.hcjd;
      this.formDataDa.xmlx = data.xmlx;
      this.formDataDa.zglsqk = data.zglsqk;
      this.formDataDa.qtjtqx = data.qtjtqx;
      this.formDataDa.hfjtqx = data.hfjtqx;
      this.formDataDa.wfsjyt = data.sjyt;

      const updateFormData = (field, value, condition) => {
        switch (condition) {
          case "3":
            this.formDataDa[field] = value;
            break;
          case "1":
            this.formDataDa[`${field}Max`] = value;
            break;
          case "2":
            this.formDataDa[`${field}Min`] = value;
            break;
          case "4":
            this.formDataDa[`dydy${field}`] = value;
            break;
          case "5":
            this.formDataDa[`xydy${field}`] = value;
            break;
          case "6":
            const [min, max] = value.split("~");
            this.formDataDa[`${field}Max`] = max;
            this.formDataDa[`${field}Min`] = min;
            break;
        }
      };
      ...
      if (data.freck.condition === "4") {
        this.formDataDa.dydygdmj = data.freck.value;
      } else if (data.freck.condition === "5") {
        this.formDataDa.xydygdmj = data.freck.value;
      } else {
        updateFormData("gdmj", data.freck.value, data.freck.condition);
      }

      if (data.Cufreck.condition === "4") {
        this.formDataDa.dydynydmj = data.Cufreck.value;
      } else if (data.Cufreck.condition === "5") {
        this.formDataDa.xydynydmj = data.Cufreck.value;
      } else {
    ...
    terDialog() {
      if (this.namerow == "") {
        ...
        this.formDataDa.remarks = this.namerow;
        let data = { ...this.formDataDa };
    ...
    exportData() {
      ...
      special2022cjhh(this.formDataDa)
        .then((res) => {
          if (res.code === 200) {
    ...
    handleCurrentChange(value) {
      this.formDataDa.pageNum = value;
      this.findAll();
    },
    handleSizeli(value) {
      this.formDataDa.pageSize = value;
      this.findAll();
    },
    ...
    handleChangeXzq(value) {
      this.formDataDa.city = "";
      this.formDataDa.county = "";
      if (value.length) {
        const textArr = [];
        ...
        this.formDataDa.city = arr.join(",");
        this.formDataDa.county = county.join(",");
      }
    },
    //数组转对象
    arrayToObject(arr) {
    ...
    findAll(isSearch) {
      ...
      // }
      if (this.formDataDa.snapdate) {
        this.formDataDa.snapdate = this.formatDate(this.formDataDa.snapdate);
      }

      const { selectXZQ, valuetime, ...params } = this.formDataDa;
