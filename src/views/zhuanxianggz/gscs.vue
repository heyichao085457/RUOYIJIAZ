<template>
  <div class="container">
    <div class="line"></div>
    <div class="sellect">
      <el-form ref="form" :model="formDatauop" label-width="55px">
        <div>
          <el-row class="title" type="flex" align="middle">
            ...
            <el-col :span="4" :style="{ marginLeft: '15px' }">
              <el-input
                size="medium"
                placeholder="请输入图斑编号"
                v-model.trim="formDatauop.dkOrTb"
                clearable
...
export default {
  ...
  data() {
    return {
      ...
      urlList: [],
      urlpo: [],
      fieldsToClear: ["tbmjMin", "tbmjMax", "tbmj", "dydytbmj", "xydytbmj"],
    };
  },
  created() {
  ...
  methods: {
    ...
    clearFormData() {
      this.formDatauop.tbmjMin = "";
      this.formDatauop.tbmjMax = "";
      this.formDatauop.tbmj = "";
      this.formDatauop.dydytbmj = "";
      this.formDatauop.xydytbmj = "";
    },
    onSubprimary() {
      ...
      switch (this.customValue.condition) {
        case "1":
          this.formDatauop.tbmjMax = this.customValue.value;
          break;
        case "2":
          this.formDatauop.tbmjMin = this.customValue.value;
          break;
        case "3":
          this.formDatauop.tbmj = this.customValue.value;
          break;
        case "4":
          this.formDatauop.dydytbmj = this.customValue.value;
          break;
        case "5":
          this.formDatauop.xydytbmj = this.customValue.value;
          break;
        case "6":
          if (this.customValue.value && this.customValue.value.includes("~")) {
            const parts = this.customValue.value.split("~");
            this.formDatauop.tbmjMin = parts[0].trim();
            this.formDatauop.tbmjMax = parts[1].trim();
          }
          break;
      }
      this.findAll();
    ...
    saveFormData() {
      localStorage.setItem("formDatauop", JSON.stringify(this.formDatauop));
    },
    loadFormData() {
      const savedData = localStorage.getItem("formDatauop");
      if (savedData) {
        this.formDatauop = JSON.parse(savedData);
      }
    },
    ...
    beforeRouteEnter(to, from, next) {
      next((vm) => {
        if (to.params.savedData) {
          vm.formDatauop = to.params.savedData;
        }
      });
    },
    handleDateChange(dates) {
      if (Array.isArray(dates) && dates.length > 0) {
        ...
        this.formDatauop.startDate = formattedDates[0];
        this.formDatauop.endDate = formattedDates[1];
      } else {
        this.formDatauop.startDate = "";
        this.formDatauop.endDate = "";
      }
    },
    emitGetData(data) {
      console.log(data);
    ...
    terDialog() {
      if (this.namerow == "") {
        ...
        this.formDatauop.remarks = this.namerow;
        let data = { ...this.formDatauop };
    ...
    exportData() {
      ...
      special2022wscs(this.formDatauop)
        .then((res) => {
          if (res.code === 200) {
    ...
    handleCurrentChange(value) {
      this.formDatauop.pageNum = value;
      this.findAll();
    },
    handleSizeli(value) {
      this.formDatauop.pageSize = value;
      this.findAll();
    },
    ...
    handleChangeXzq(value) {
      this.formDatauop.city = "";
      this.formDatauop.county = "";
      if (value.length) {
        const textArr = [];
        ...
        this.formDatauop.city = arr.join(",");
        this.formDatauop.county = county.join(",");
      }
    },
    //数组转对象
    arrayToObject(arr) {
    ...
    findAll(isSearch) {
      ...
      // }
      if (this.formDatauop.snapdate) {
        this.formDatauop.snapdate = this.formatDate(this.formDatauop.snapdate);
      }

      const { selectXZQ, valuetime, ...params } = this.formDatauop;
