  ...
  methods: {
    ...
    beforeRouteEnter(to, from, next) {
      next((vm) => {
        if (to.params.savedData) {
          vm.formData = to.params.savedData;
        }
      });
    },
    handleDateChange(dates) {
      if (Array.isArray(dates) && dates.length > 0) {
        ...
        this.formData.startDate = formattedDates[0];
        this.formData.endDate = formattedDates[1];
      } else {
    ...
    emitGetData(data) {
      ...
      this.formData.pdlx = data.tblx;
      this.formData.shengjidxjg = data.checkMethod;
      this.formData.shijidxjg = data.illegalityType;
      this.formData.qbtbHchpdlx = data.hchlxx;
      this.formData.qbtbSfnrwfjs = data.deductionType;
      this.formData.qbtbTbly = data.tbly;
      this.formData.tbfl = data.tbfl;
      this.formData.qbtbCancelStatus = data.qbtbCancelstatus;
      ...
      const updateFormData = (field, value, condition) => {
        switch (condition) {
          case "3":
            ...
            break;
          case "1":
            this.formData[`${field}Max`] = value;
            break;
          case "2":
            this.formData[`${field}Min`] = value;
            break;
          case "4":
            this.formData[`dydy${field}`] = value;
            break;
          case "5":
            this.formData[`xydy${field}`] = value;
            break;
          case "6":
            const [min, max] = value.split("~");
            this.formData[`${field}Max`] = max;
            this.formData[`${field}Min`] = min;
            break;
        }
      };
      ...
      if (data.freck.condition === "4") {
        this.formData.dydygdmj = data.freck.value;
      } else if (data.freck.condition === "5") {
        this.formData.xydygdmj = data.freck.value;
      } else {
        updateFormData("gdmj", data.freck.value, data.freck.condition);
      }

      if (data.Cufreck.condition === "4") {
        this.formData.dydynydmj = data.Cufreck.value;
      } else if (data.Cufreck.condition === "5") {
        this.formData.xydynydmj = data.Cufreck.value;
      } else {
    ...
    terDialog() {
      if (this.namerow == "") {
        ...
        this.formData.remarks = this.namerow;
    ...
    findAll(isSearch) {
      ...
      const { selectXZQ, valuetime, ...params } = this.formData;
